"""Deliver Chinese briefing + audio to Telegram."""
...
logger = logging.getLogger("deliver_chinese")
...
async def main():
    ...
    briefing_path = data_dir / "artifacts" / "briefings" / f"{today}-zh.md"
    audio_path = data_dir / "artifacts" / "audio" / f"{today}-zh.mp3"

    if not briefing_path.exists():
        logger.error(f"No Chinese briefing at {briefing_path}")
        return
    ...
    logger.info(f"Delivering Chinese briefing ({len(text)} chars) + audio ({audio_info}) to {chat_id}")
